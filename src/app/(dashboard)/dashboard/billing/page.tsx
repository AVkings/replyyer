...
export default function Billing() {
  ...
  async function buy(pack_id: string) {
    ...
    const rzp = new window.Razorpay({
      ...
      handler: () => { setMsg("Payment captured — credits will appear shortly"); setTimeout(load, 3000); },
  ...
  async function validateCoupon() {
    ...
    const off = j.amount_off_paise != null ? `₹${(j.amount_off_paise / 100).toFixed(0)} off` : j.percent ? `${j.percent}% off` : "discount";
    setMsg(`Coupon ${j.code}: ${off}`);
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="rounded-2xl border border-zinc-200 bg-white p-5">
        ...
        <div className="text-xs text-zinc-500">1 credit = 1 message. 180 free on first business. Extra business = 100 credits.</div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }} className="rounded-2xl border border-zinc-200 bg-white p-5">
        <div className="text-sm font-semibold">Buy credits — secure checkout</div>
        <div className="mt-1 text-xs text-zinc-500">Try coupons: <code>FLAT600</code> (₹600 off), <code>WELCOME500</code> (₹500 off)</div>
        <div className="mt-3 flex gap-2">
          <input value={coupon} onChange={(e) => setCoupon(e.target.value)} placeholder="Coupon code (e.g. FLAT600)" className="rounded-xl border border-zinc-200 px-3 py-2 text-sm" />
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          {PACKS.map((p) => (
            <motion.div key={p.id} whileHover={{ y: -2 }} className="rounded-2xl border border-zinc-200 p-4">
              ...
              <motion.button whileTap={{ scale: 0.98 }} onClick={() => buy(p.id)} className="mt-3 w-full rounded-full bg-black py-2 text-xs font-medium text-white">Secure checkout</motion.button>
            </motion.div>
          ))}
        </div>
